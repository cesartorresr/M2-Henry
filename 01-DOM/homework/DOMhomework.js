...
// Tu código acá:
// document.querySelector("#createdBy").innerHTML ='Aplicación creada por César DIOS'

let selector = document.querySelector('#createdBy')
selector.innerHTML = 'Aplicación creada por César Dios'
...
function buildToDo(todo, index) {
  ...
  // que hace esste codigo: <span id="4"> comprar leche  </ span >
}
...
let nuevoArrayDeToDoDivs = toDos.map(buildToDo)
return nuevoArrayDeToDoDivs;
}
...
function displayToDos() {
  ...
  let arrayDeDivs = buildToDos(toDoItems)

  arrayDeDivs.forEach(function(toDoDiv, indice) {
...
function addToDo() {
 ...
 if(!input.value) return alert('No se puede crear una tarea vacía');
...
function completeToDo(event) {
  ...
  const idDelToDo = event.target.id;
  // Tu código acá:
  toDoItems[idDelToDo].completeToDo()
